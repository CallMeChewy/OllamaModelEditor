...
def Main():
    ...
    try:
        from Core.ModelManagerExtensions import ExtendModelManager
        ModelMgr = ModelManager(Config)
        # Extend ModelManager with preset handling methods
        ExtendModelManager(ModelMgr)
        Logger.info("Model manager created and extended")
    except Exception as Error:
        ShowErrorAndExit("Failed to create model manager", Error)
    
    # Create parameter state manager
    try:
        StateManager = ParameterStateManager(ModelMgr, Config)
        Logger.info("Parameter state manager created")
    except Exception as Error:
        ShowErrorAndExit("Failed to create parameter state manager", Error)
